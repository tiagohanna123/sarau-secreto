...
export function ProductCard({ produto, index }: { produto: Produto; index: number }) {
  return (
    ...
    >
      {/* Image placeholder */}
      <div className="aspect-[4/3] rounded-xl mb-4 art-placeholder border border-border/50 flex items-center justify-center overflow-hidden group-hover:border-gold/15 transition-colors duration-500">
        {produto.imagem ? (
          <img src={produto.imagem} alt={produto.nome} className="w-full h-full object-cover" />
        ) : (
          <div className="text-center p-4 relative z-10">
            <ShoppingBag size={24} className="text-gold-dim/20 mx-auto mb-1.5 group-hover:text-gold-dim/40 transition-colors duration-500" />
      ...
      <h3 className="text-sm font-display font-light text-foreground mb-1 group-hover:text-gold transition-colors duration-300">
        {produto.nome}
      </h3>
      ...
      <div className="flex items-center justify-between gap-2 mt-auto pt-3 border-t border-border/20">
        <span className="text-sm font-medium text-gold tracking-wide">{produto.preco}</span>
