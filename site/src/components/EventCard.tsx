...
const statusLabels: Record<string, { label: string; class: string }> = {
  ...
  breve:      { label: 'Em Breve', class: 'bg-gold-glow text-gold border-gold/15' },
...
export function EventCard({ evento, index }: { evento: Evento; index: number }) {
  ...
  return (
    ...
    >
      {/* Hover glow */}
      <div className="absolute -inset-1 bg-gradient-to-r from-gold/0 via-gold/[0.02] to-violet/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />

      <div className="relative">
        <div className="flex items-start justify-between gap-4 mb-3">
          <div className="flex items-center gap-3">
            <span className="text-[0.55rem] font-mono text-gold-dim/50 font-light">
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground mb-4">
          {evento.data && (
            <span className="flex items-center gap-1.5">
              <Calendar size={12} className="text-gold-dim/60" />
          ...
          {evento.local && (
            <span className="flex items-center gap-1.5">
              <MapPin size={12} className="text-gold-dim/60" />
              {evento.local}
            </span>
          )}
        </div>
