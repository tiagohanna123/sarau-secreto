  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 animate-pulse">
      ...
      <div className="space-y-3">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="kpi-card h-36" />
        ))}
      </div>
    </div>
  )
          ...
          <div className="flex items-center gap-1 text-[10px]">
            ...
            {(['date', 'revenue', 'bar', 'perCapita'] as const).map(s => (
              <button key={s} onClick={() => setSortBy(s)}
                className={`px-2 py-1 rounded transition-colors ${sortBy === s ? 'bg-[#c8a96e]/20 text-[#c8a96e]' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      <div className="mb-6 grid grid-cols-4 gap-3">
        {[
          ...
          <div key={kpi.label} className="kpi-card text-center">
            ...
            <p className="mt-1 text-lg font-bold text-[#c8a96e]">{kpi.value}</p>
        ...
        {sorted.map((ev) => {
          ...
          return (
            <button
              ...
              className={`kpi-card w-full text-left transition-all duration-200 ${
                hovered === ev.id ? 'border-[#c8a96e]/40 bg-white/5' : ''
              }`}
            >
              ...
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] font-medium uppercase tracking-widest text-[#8b5cf6]">
                      {ev.dateLabel}
                    </span>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-white/[0.03] px-3 py-2">
                  <p className="text-[9px] uppercase tracking-wider text-muted-foreground">
                    Bar {barPct > 0 && <span className="text-[#c8a96e]">({barPct}%)</span>}
                  </p>
                  <p className="mt-0.5 text-sm font-bold text-[#c8a96e]">{fmtBRL(ev.barRevenue)}</p>
                </div>
                ...
                {ev.total > 0 && (
                  <div className="flex items-center gap-1.5">
                    <div className="flex h-2 w-20 overflow-hidden rounded-full bg-white/10">
                      ...
                      <div className="h-full rounded-r-full bg-[#c8a96e] transition-all"
                        style={{ width: `${barPct}%` }} />
                    </div>
